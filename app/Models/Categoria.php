        ...
        return $db->fetch(
            "SELECT cat.*,
                    (SELECT COUNT(DISTINCT cc.comercio_id)
                     ...
                     INNER JOIN comercios c ON cc.comercio_id = c.id AND c.activo = 1                        AND (c.plan_fin IS NULL OR c.plan_fin >= CURDATE())
    ...
    public static function getWithComerciosCount(): array
    {
        ...
        return $db->fetchAll(
            "SELECT cat.*,
             ...
             LEFT JOIN comercios c ON cc.comercio_id = c.id AND c.activo = 1                AND (c.plan_fin IS NULL OR c.plan_fin >= CURDATE())
             WHERE cat.activo = 1
             GROUP BY cat.id

...
6. **Sitemap dinámico, NO estático** (cambio 2026-05-02 bloques 3, 3.1, 11): generado por `app/Services/SitemapService.php` y servido por `app/Controllers/Public/HomeController::sitemap()`. La URL `/sitemap.xml` está forzada al front controller via `.htaccess` (`RewriteRule ^sitemap\.xml$ index.php [L,QSA]`) — Apache NO sirve el archivo en disco directamente. Cache de **1 hora**: si el archivo en disco tiene <1h se sirve sin regenerar; si excede, la primera petición regenera y persiste a disco. Excluye categorías huérfanas (sin comercios activos) y fechas con redirect 301 activo en `seo_redirects`. Incluye home, listados estáticos, páginas legales, categorías con contenido, comercios activos, productos publicados, noticias y fechas vigentes. **NO está trackeado en git** (`/sitemap.xml` en `.gitignore` desde commit `46c49a2`) — los deploys ya NO sobrescriben el archivo regenerado. Backup forense del último estado bueno: `/home/purranque/backups/2026-05-02/sitemap.before-block11.bak`.
...
9. **Comportamiento del deploy de cPanel** (verificado 2026-05-02): el flujo "Update from Remote → Deploy HEAD Commit" parece ejecutar el `cp -R` con el workdir en su estado PREVIO al pull, y solo después actualiza el workdir al nuevo HEAD. Esto significa que: (1) el primer deploy tras un cambio que destrackea un archivo aún sobrescribe ese archivo en producción una última vez, (2) los deploys subsiguientes ya no lo tocan. Implicación práctica: cuando se destrackee un archivo (como hicimos con `sitemap.xml`), el primer deploy puede dejar producción en estado intermedio incorrecto durante ~1h hasta que el auto-regen dispare.
